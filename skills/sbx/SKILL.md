---
name: sbx
description: Load to know about the environment you're running in, when making changes to shell, when something is broken or network access is blocked 
---
...
- The multi-line form is emitted for ungoverned implicit / default-deny cases
  (governed implicit denials use the `Blocked by org policy` form above instead):
...
**Protocols**: Supported values are `tcp4` (the default), `tcp`, `tcp6`, `udp`, `udp4`, and
`udp6`. The default binds IPv4 only, which is what a service listening on `0.0.0.0` needs; pass
`tcp` to bind both address families, or `tcp6` if the service listens exclusively on IPv6. A
dual-stack `tcp` binding forwards an IPv6 connection to the sandbox's IPv6 address, so a service
listening only on IPv4 accepts and then resets it — prefer the default unless the service listens
on `::`. The same applies to `udp`/`udp4`/`udp6`.
...
Supported non-local fetch remotes from the host's Git config are available in the sandbox under
the **same name** (`origin`, `upstream`, a `fork`, …), with embedded credentials removed. Local,
push-only, helper-backed, custom-transport, and ambiguous remotes are not copied. Check
`git remote -v` before relying on a host remote. The normal Git flow works as-is in both
workspace modes:
